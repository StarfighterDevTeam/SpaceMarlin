...
void main()
{
	...
	fragHeight = 10*max(0, -0.7 + 1-length(vec2(varUv.x+0.1, varUv.y+0.2)*2-1));
	//fragHeight = 0;
	...
	//	gTerm3.x * texture(texHeights1, uv + gTexelSize * vec2(-1, 0)).r +
	//	gTerm3.x * texture(texHeights1, uv + gTexelSize * vec2(+1, 0)).r +
	//	gTerm3.y * texture(texHeights1, uv + gTexelSize * vec2(0, -1)).r +
	//	gTerm3.y * texture(texHeights1, uv + gTexelSize * vec2(0, +1)).r;
