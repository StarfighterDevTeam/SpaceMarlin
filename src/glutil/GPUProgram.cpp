...
void GPUProgram::bindAttribLocations(const GPUProgram::Location* locations, int nb_locations)
{
...
void GPUProgram::bindFragDataLocations(const GPUProgram::Location* locations, int nb_locations)
{
	for(int i=0 ; i < nb_locations ; i++)
...
void GPUProgram::setUniformNames(const char* uniform_name, ...)
{
	...
	nb_uniforms = (int)uniform_list.size();
...
void GPUProgram::setUniformNames(const char** uniform_names, int nb_uniforms)
{
	// Delete previous uniforms
...
void GPUProgram::setUniformNames(const string* uniform_names, int nb_uniforms)
{
	// Delete previous uniforms
	delete [] this->uniforms;
...
void GPUProgram::setUniformNames(const list<string>& uniform_names)
{
	...
	nb_uniforms = (int)uniform_names.size();
	uniforms = new Uniform[nb_uniforms];
...
void GPUProgram::sendUniform(const char* uniform_name, const mat3& m,	Hash::Marker marker, bool transpose) const
{
	GLint location = getUniformLocation(Hash(uniform_name, Hash::AT_RUNTIME));
	CHECK_LOCATION(uniform_name, location);
...
void GPUProgram::sendUniform(const char* uniform_name, const mat4& m,	Hash::Marker marker, bool transpose) const
{
	GLint location = getUniformLocation(Hash(uniform_name, Hash::AT_RUNTIME));
	CHECK_LOCATION(uniform_name, location);
