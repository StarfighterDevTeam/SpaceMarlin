...
private:
	...
	int for_directive_counter;	// Counter for #for.
	int for_directive_nb_iter;	// Number of iterations
	int for_directive_num_line;	// Number of the line where the #for directive was
...
public:
	...
	int getNbSymbols() const;

#ifdef PREPROC_KEEP_ORIGINAL_SYMBOLS
	const SymbolList& getOriginalSymbols() const;
	int getNbOriginalSymbols() const;
#endif
...
private:
	...
	bool forExpression(const char* directive, const char* filename, int num_line);
	bool endFor(const char* directive, const char** new_line_ptr, std::string* result, int* next_num_line);
};
