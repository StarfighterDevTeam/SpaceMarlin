...
{
	int nb_symbols = (int)symbols.size();
...
int Preprocessor::getNbSymbols() const
{
...
int Preprocessor::getNbOriginalSymbols() const
{
	return (int)(this->original_symbols.size());
}
#endif
...
const char* Preprocessor::removeComments(const char* original_source, const char* filename)
{
	...
	for(int i=0 ; original_source[i] != '\0' ; i++)
	{
		...
		{
			int num_line_comment_start = num_line;
...
string Preprocessor::preprocessLine(const char* line,
									...
									int* num_line,
									list<bool>* commented_out,
									int *nb_generated_lines,
									const char** new_line_ptr)
{
	// Preprocess line:
	string result;

	int next_num_line = (*num_line) + 1;
								...
								int* nb_generated_lines)
{
	...
	*nb_generated_lines = 1;
	...
	// - go to the first '"'
	for(i=(int)strlen("#include") ; directive[i] != '\"' ; i++)
	{
		char c = directive[i];
		if(!isWhitespace(c) && c != '\"')
			return false;
	...
	int prev_source_string_number = source_string_number;
...
{
	PreprocSym symbol;
	int keyword_len = (int)strlen("#define");
...
bool Preprocessor::undefSymbol(const char* directive)
{
	...
	int keyword_len = (int)strlen("#undef");
...
bool Preprocessor::condExpression(const char* directive, bool* is_true)
{
	...
	if(startsWith(directive, "#if"))
		keyword_len = (int)strlen("#if");
	else
		keyword_len = (int)strlen("#else");
...
bool Preprocessor::definedExpression(const char* directive, bool* is_defined)	// for #ifdef and #ifndef
{
	...
	if(startsWith(directive, "#ifdef"))
		keyword_len = (int)strlen("#ifdef");
	else
		keyword_len = (int)strlen("#ifndef");
...
// Input example:
// "#for NB_LIGHTS"
bool Preprocessor::forExpression(const char* directive, const char* filename, int num_line)
{
	string str_expression = "";
	...
	const int keyword_len = (int)strlen("#for");
	if(!isWhitespace(directive[keyword_len]))
		return false;
	...
	this->for_directive_nb_iter = value;
...
// Input example:
// "#endfor"
bool Preprocessor::endFor(const char* directive, const char** new_line_ptr, string* result, int* next_num_line)
{
